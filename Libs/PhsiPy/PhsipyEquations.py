...
class Subatomic:
    ...
    def binding_energy(mass_parent, mass_daughters):
        return (mass_parent - sum(mass_daughters)) * (
            9e16
...
class Electromagnetism:
    ...
    @staticmethod
    def hall_voltage(
        magnetic_field_strength, current, charge_density, electron_density
    ):
    ...
    def compton_wavelength_change(initial_wavelength, scattering_angle):
        return initial_wavelength - (
            compton_wavelength * (1 - math.cos(scattering_angle))
        )

    @staticmethod
    def bohr_orbit_radius(principal_quantum_number):
        return (
            (finestructure_constant**2)
            * (permittivity_of_free_space * (plancks_constant**2))
            / (
                (math.pi * elementary_charge**2)
    ...
    def maxwell_equation(magnetic_field, magnetic_flux_density):
        return (
            np.cross(magnetic_field, magnetic_flux_density)
            - permittivity_of_free_space * magnetic_field
        )
    ...
    def photoelectric_effect_max_velocity(work_function, stopping_potential):
        return math.sqrt(
            (2 * elementary_charge * (stopping_potential - work_function))
            / Subatomic.Mass.electron
        )
...
class Thermodynamics:
    ...
    def heat_transfer_radiation(
        emissivity, stefan_boltzmann_constant, area, temperature1, temperature2
    ):
        return (
            emissivity
    ...
    @staticmethod
    def entropy_change_adiabatic(
        reversible_entropy_change, heat_transfer, temperature_reservoir
    ):
...
class QuantumMechanics:
    ...
    def uncertainty_principle(position_uncertainty, momentum_uncertainty):
        return position_uncertainty * momentum_uncertainty >= plancks_constant / (
            4 * pi
        )
    ...
    def tunneling_probability(energy_barrier, particle_mass, particle_energy):
        return math.exp(
            (-2 * math.sqrt(2 * particle_mass * energy_barrier) / plancks_constant)
    ...
    def black_body_radiation_intensity(frequency, temperature):
        return (2 * plancks_constant * frequency**3) / (
            ...
            * (
                math.exp(
                    (plancks_constant * frequency) / (boltzmann_constant * temperature)
                )
                - 1
            )
        )
    ...
    def black_body_radiation_power(temperature):
        return (2 * math.pi**5 * boltzmann_constant**4 * temperature**4) / (
    ...
    def fermi_dirac_distribution(energy, fermi_level, temperature):
        return 1 / (
            math.exp((energy - fermi_level) / (boltzmann_constant * temperature)) + 1
        )

    @staticmethod
    def bose_einstein_distribution(energy, chemical_potential, temperature):
        return 1 / (
            math.exp((energy - chemical_potential) / (boltzmann_constant * temperature))
            - 1
        )
    ...
    @staticmethod
    def heisenberg_uncertainty_position_momentum(
    ...
    def atomic_orbital_radius(principal_quantum_number):
        return (
            ...
            * (plancks_constant**2)
            * (principal_quantum_number**2)
        ) / (Subatomic.Mass.electron * (elementary_charge**2))

    @staticmethod
    def fine_structure_constant():
        return (elementary_charge**2) / (
            4 * pi * permittivity_of_free_space * plancks_constant * speed_of_light
        )
    ...
    def plank_distribution_radiation(energy, temperature):
        return (2 * plancks_constant * (energy**3)) / (
            ...
            * (
                math.exp(
                    (plancks_constant * energy) / (boltzmann_constant * temperature)
                )
                - 1
            )
        )
    ...
    def plank_law_power(temperature):
        return (2 * math.pi * (boltzmann_constant**4) * temperature**4) / (
            15 * (plancks_constant**3) * (speed_of_light**2)
        )
    ...
    def atomic_spectra(
        rydberg_constant,
        atomic_number,
        principal_quantum_number_initial,
        principal_quantum_number_final,
    ):
        return (rydberg_constant * atomic_number**2) * (
            (1 / principal_quantum_number_final**2)
            - (1 / principal_quantum_number_initial**2)
        )

    @staticmethod
    def absorption_spectrum(frequency_emitted, frequency_absorbed, velocity):
        return (
            frequency_absorbed - frequency_emitted
        ) / frequency_emitted == velocity / speed_of_light

    @staticmethod
    def emission_spectrum(frequency_emitted, frequency_absorbed, velocity):
        return (
            frequency_absorbed - frequency_emitted
        ) / frequency_absorbed == velocity / speed_of_light
...
class SolidStatePhysics:
    ...
    def fermi_energy(electron_density, work_function):
        return (electron_density * (plancks_constant**2)) / (
            2 * Subatomic.Mass.electron
        ) + work_function
    ...
    def intrinsic_carrier_concentration(
        ...
        acceptor_density,
        fermi_energy,
        intrinsic_fermi_level,
        temperature,
    ):
        return ((donor_density * acceptor_density) ** 0.5) * math.exp(
            -(fermi_energy - intrinsic_fermi_level)
    ...
    def depletion_layer_width(fermi_energy, potential_difference, electron_density):
        return math.sqrt(
            (2 * permittivity_of_free_space * potential_difference)
            / (Subatomic.Charge.electron * electron_density)
        ) / (2 * fermi_energy)
    ...
    def superconductivity_critical_temperature(critical_field):
        return (critical_field**2) * (
            permittivity_of_free_space
            / (2 * Subatomic.Mass.electron * Subatomic.Charge.electron)
        )
    ...
    def superconductivity_coherence_length(hamiltonian_operator, fermi_energy):
        return (plancks_constant / (2 * math.pi)) * (
            ...
            / (2 * Subatomic.Mass.electron * fermi_energy)
        ) ** 0.5
...
class FluidStatePhysics:
    ...
    @staticmethod
    def surface_tension(
        surface_tension_coefficient, contact_angle, radius_of_curvature
    ):
        return (
            surface_tension_coefficient * math.cos(contact_angle) / radius_of_curvature
        )
    ...
    def bernoullis_equation(
        static_pressure, dynamic_pressure, height, density, gravitational_acceleration
    ):
        return (
            static_pressure
            + dynamic_pressure
            + density * gravitational_acceleration * height
        )
    ...
    @staticmethod
    def charles_law(
        initial_volume, final_volume, initial_temperature, final_temperature
    ):
        return (initial_volume * final_temperature) / (
            final_volume * initial_temperature
        )

    @staticmethod
    def gaylussacs_law(
        initial_pressure, final_pressure, initial_temperature, final_temperature
    ):
        return (initial_pressure * final_temperature) / (
            final_pressure * initial_temperature
        )
